    ...
    def initiate_model_training(self, train_data_path, test_data_path):
        try:
            ...
            sorted_models = sorted(
                model_score.items(), key=lambda x: x[1], reverse=True
            )
            ...
            for model_name, score in top_3_models:
                with mlflow.start_run(run_name=model_name):
                    ...
                    pipeline = Pipeline(
                        [("preprocessor", preprocessor), ("model", tuned_model)]
                    )
            ...
            os.makedirs(
                os.path.dirname(self.model_config.train_model_path), exist_ok=True
            )
